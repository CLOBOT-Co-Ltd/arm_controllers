    ...
    : Node("arm_controller_node")
    {
        ...
        low_state_subscriber_.reset(
            new unitree::robot::ChannelSubscriber<unitree_hg::msg::dds_::LowState_>(
                kTopicState));
        low_state_subscriber_->InitChannel(std::bind(&ArmActionServerNode::on_lowstate_received, this, std::placeholders::_1), 1);
        ...
        target_pos_ = {Pi_2 / 2, deg_20, 0, Pi_2 / 2, 0, 0, 0,
        ...
        action_server_ = rclcpp_action::create_server<arm_interfaces::action::Gesture>(
            ...
            std::bind(&ArmActionServerNode::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
            std::bind(&ArmActionServerNode::handle_cancel, this, std::placeholders::_1),
            std::bind(&ArmActionServerNode::handle_accepted, this, std::placeholders::_1));
...
private:
    ...
    std::array<float, 15> target_pos_;
    ...
    void move_arm_to_pose(
        ...
        float duration_sec,
        const std::shared_ptr<rclcpp_action::ServerGoalHandle<arm_interfaces::action::Gesture>> goal_handle,
        const std::shared_ptr<arm_interfaces::action::Gesture::Feedback> feedback) // Accept feedback pointer
    {
        ...
        for (int i = 0; i <= num_time_steps; ++i) {
            ...
            for (int j = 0; j < arm_joints_.size(); ++j) {
                ...
                current_jpos_des.at(j) = start_pos.at(j) * (1.0f - phase) + target_pose.at(j) * phase;
                ...
                if (feedback) { // Use the passed feedback pointer
                   ...
                   feedback->current_joint_angle.at(j).joint_idx.name = "joint_" + std::to_string(arm_joints_.at(j)); // Example name
                   feedback->current_joint_angle.at(j).joint_idx.number = arm_joints_.at(j);
                   feedback->current_joint_angle.at(j).angle_rad = state_msg_.motor_state().at(arm_joints_.at(j)).q(); // Use actual state for feedback
                }
            }
        ...
        for (int i = 0; i <= num_time_steps; ++i) {
            ...
            for (int j = 0; j < arm_joints_.size(); ++j) {
                ...
                current_jpos_des.at(9) = start_pos.at(9) * (1.0f - phase) + (deg_30 * std::sin(4 * Pi * phase)) * phase;
         ...
         for (int i = 0; i < arm_joints_.size(); ++i) {
             ...
             if(state_msg_.motor_state().size() > arm_joints_.at(i)) {
                current_jpos.at(i) = state_msg_.motor_state().at(arm_joints_.at(i)).q();
                 ...
                 RCLCPP_WARN(this->get_logger(), "Motor state not available for joint %d during initialization. Using 0.0.", arm_joints_.at(i));
        ...
        std::shared_ptr<const arm_interfaces::action::Gesture::Goal> goal)
    {
        ...
        if (goal->action == 0 || goal->action == 1) {
    ...
    void handle_accepted(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arm_interfaces::action::Gesture>> goal_handle)
    {
        ...
        std::thread{std::bind(&ArmActionServerNode::execute, this, std::placeholders::_1), goal_handle}.detach();
    }


    // Action execution
    void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arm_interfaces::action::Gesture>> goal_handle)
    {
        ...
        } else if (goal->action == 1) {
            target_pose = target_pos_; // Action 1: Move to target_pos (arms up)
             RCLCPP_INFO(this->get_logger(), "Action 1: Moving to target pose (arms up).");
        } else {
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<ArmActionServerNode>(argv[1]);
